...
from jaxopt import OptaxSolver as _OptaxSolver
...
@dataclass(eq=False)
class OptaxSolver(_OptaxSolver):
    callback: None | Callable[[jaxopt.OptStep], None] = None
    callback_every: int = 1

    def __post_init__(self):
        super().__post_init__()

        def tab_fn(step, transforms):
            if self.callback is not None:
                self.callback(step)
                
        def cb(step):
            hcb.id_tap(tab_fn, step)
            
        self._cb = cb
        if self.jit:
            self.update = jit(self.update)
    
    def update(self, params: Any, state: NamedTuple, *args, **kwargs) -> jaxopt.OptStep:
        step = super().update(params, state, *args, **kwargs)
        if self.callback is not None: 
            lax.cond(
                step.state.iter_num % self.callback_every == 0,
                lambda: self._cb(step),
                lambda: None
            )
        return step
