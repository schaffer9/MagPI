...
class TestQuaternion(JaxTestCase):
    def test_000_init(self):
        ...
        self.assertIsclose(q.real, 1.)
        self.assertIsclose(q.imag, array((0., 0., 0.)))
    ...
    def test_001_add(self):
        ...
        q = q1 + q2
        self.assertIsclose(q.real, 1.)
        self.assertIsclose(q.imag, array((1., 1., 1.)))

    def test_002_add(self):
        q = Quaternion(1., (0., 0., 0.)) + 1
        self.assertIsclose(q.real, 2.)
        self.assertIsclose(q.imag, array((1., 1., 1.)))
    ...
    def test_006_mul(self):
        ...
        q = q1 * q2
        self.assertIsclose(q.real, 0.)
        self.assertIsclose(q.imag, array((0., 0., 1.)))
    ...
    def test_007_mul(self):
        ...
        q = q1 * q2
        self.assertIsclose(q.real, 0.)
        self.assertIsclose(q.imag, array((0., 1., 0.)))
    ...
    def test_008_mul(self):
        ...
        q = q1 * q2
        self.assertIsclose(q.real, 0.)
        self.assertIsclose(q.imag, array((2., 0., 0.)))

    def test_009_mul_scalar(self):
        q = Quaternion(1., (2., 2., 2.)) * 2
        self.assertIsclose(q.real, 2.)
        self.assertIsclose(q.imag, array((4., 4., 4.)))

    def test_010_resciprocal(self):
        q = Quaternion(1., (2., 2., 2.))
        self.assertIsclose(abs(q * q.reciprocal()), 1.)

    def test_011_conj(self):
        q = Quaternion(1., (2., 2., 2.))
        qconj = Quaternion(1., (-2., -2., -2.))
    ...
    def test_018_rotate_point(self):
        ...
        q = from_axis_angle(pi / 2, (1., 0., 0.))
        y = quaternion_rotation(p, q)
        self.assertIsclose(y, array((1., 0., 0.)))
        
        q = from_axis_angle(pi / 2, (0., 1., 0.))
        y = quaternion_rotation(p, q)
        self.assertIsclose(y, array((0., 0., 1.)))
    
        q = from_axis_angle(pi / 2, (0., 0., 1.))
        y = quaternion_rotation(p, q)
        self.assertIsclose(y, array((0., -1., 0.)))

    def test_019_rotate_euler_angles(self):
        p = array([1., 0., 0.])
        q = from_euler_angles((pi / 2., 0., 0.))
        y = quaternion_rotation(p, q)
        self.assertIsclose(y, array((1., 0., 0.)))

        q = from_euler_angles((0., pi / 2, 0.))
        y = quaternion_rotation(p, q)
        self.assertIsclose(y, array((0., 0., 1.)))

        q = from_euler_angles((0., 0., pi / 2))
        y = quaternion_rotation(p, q)
        self.assertIsclose(y, array((0., -1., 0.)))
