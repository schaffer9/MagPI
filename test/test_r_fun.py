...
class TestRFunc(JaxTestCase):
    ...
    def test_05_equivalence(self):
        ...
        self.assertIsclose(df(0.), 0.)
        self.assertIsclose(df(1.), 0.)
        self.assertTrue(df(-0.1) < 0.)
        self.assertTrue(df(1.1) < 0.)

    def test_05_implication(self):
        df1 = translate(sphere(r = 1), 0.)
        df2 = translate(sphere(r = 1), 1.)
        df = r0.implication(df1, df2)
        self.assertIsclose(df(0.), 0.)
        self.assertTrue(df(1.) > 0.)
        self.assertTrue(df(1.9) > 0.)
        self.assertTrue(df(2.1) > 0.)
        self.assertTrue(df(-1.1) > 0.)
...
class TestCube(JaxTestCase):
    def test_00_cube1d(self):
        df = cube(1., centering=True)
        self.assertIsclose(df(array(-0.5)), 0.)
        self.assertIsclose(df(array(0.5)), 0.)

    def test_01_t_shape(self):
        df1 = cuboid((1, 3.5))
        df1 = translate(df1, [1., 0.])
        df2 = cuboid((3, 1))
        df2 = translate(df2, [0, 3])
        df = r0.union(df1, df2)
        self.assertIsclose(df(array([1.5, 4])), 0.)
        self.assertIsclose(df(array([1.5, 0])), 0.)
        self.assertIsclose(norm(grad(df)(array([1.5, 4]))), 1.)
        self.assertIsclose(norm(grad(df)(array([1.5, 0]))), 1.)
        self.assertGreater(df(array([1.5, 3])), 0.)
...
class TestSphere(JaxTestCase):
    def test_00_sphere(self):
        s = sphere(2.)
        y = s(array([2., 0.]))
        print(norm(grad(s)(array([2., 0.]))))
        self.assertIsclose(y, 0.)
        self.assertIsclose(norm(grad(s)(array([2., 0.]))), 1.)
        self.assertIsclose(norm(grad(s)(array([0., 2.]))), 1.)
    ...
    def test_01_sphere_normalized_1nd_order(self):
        ...
        self.assertIsclose(normal_derivative(s)(1.), 1.)
        self.assertIsclose(normal_derivative(s)(-1.), 1.)
